...
// 保存购物车信息的方法
function saveCart(productList) {
    axios.post("http://localhost:3000/carts", {
        username,
        products: productList
    }).then((rls) => {
        console.log("购物车信息保存成功", rls);
    }).catch((error) => {
        console.log("保存购物车信息失败，请检查网络", error);
    })
}

// 为保存购物车按钮添加点击事件
let saveCartBtn = document.querySelector(".btn-primary:nth-of-type(2)");
